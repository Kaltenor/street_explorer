...
- Shared authoritative light-orange exploration-surface and completion contours, with non-decreasing reprocess safety.
...
- Midnight Cartographer presentation on iOS with dark muted MapKit, hidden native POIs and location cursor, light-orange explored territory, gold current activity, restrained saved-route colors, and custom parchment-and-ink route/medal markers.
- Original hand-inked cartographer player with four directional idle poses, three restrained walking frames per direction, desaturated stale-GPS variants, and one stable opacity-switched MapKit annotation.
- Atlas Cabinet presentation across Details, History, Completion, and Options with gold-edged navy paper panels, cartographic headers, quiet page/ink sounds, Reduce Motion-aware transitions, objective stamps, explored-area ink reveal, and saved-route draw-on focus.
...
- Persistent city-context district outlines with twice-thick burnt-orange strokes, a stronger burnt-orange selected district, and a red selected city boundary, intentional haptic long-press district/city selection, direct scope buttons, objective-preserving map panning, latest-only boundary/percentage calculations, and a compact objective HUD that refreshes on newly enclosed active areas and Stop, not ordinary open-line cells, while showing completion, remaining cells, and today's added cells.
