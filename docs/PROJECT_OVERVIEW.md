...
- Map-first startup with accuracy-aware current-location centering, one persistent explicitly sized 64×64-point CC0 pixel-art player overlay, four-direction facing, automatic live-position following, projected positioning after manual map movement, native iOS location fallback, last-trustworthy-position continuity across GPS and Stop/Start transitions, an accessible stale-fix state, a self-healing idle/recording watcher, cached explored cells, and lazy saved-route loading.
...
- Persistent selected-city district outlines with a gold active boundary, intentional haptic long-press district/city selection, direct scope buttons, objective-preserving map panning, latest-only boundary/percentage calculations, and a compact objective HUD showing completion, remaining cells, and today's added cells.
