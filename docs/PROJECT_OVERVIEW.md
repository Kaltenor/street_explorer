...
- Map-first startup with accuracy-aware current-location centering, one explicitly sized 64×64-point CC0 static player annotation anchored directly to its MapKit coordinate, one-time Start/Resume recentering, camera-independent panning, native iOS location fallback, durable last-trustworthy-position continuity across GPS, Stop/Start, force-close/relaunch, and recovery transitions, an accessible stale-fix state, a self-healing idle/recording watcher, cached explored cells, and lazy saved-route loading.
...
- Persistent selected-city district outlines with a gold active boundary, intentional haptic long-press district/city selection, direct scope buttons, objective-preserving map panning, latest-only boundary/percentage calculations, and a compact objective HUD that previews active loop closures while showing completion, remaining cells, and today's added cells.
