...
25. Finish a walk containing a suspicious GPS gap and confirm its bounded street-topology lookup does not delay ordinary continuous saves. In History, verify the Street bridges totals and per-bridge Technical details; confirm an overpass is not joined to the street below. Tap Reprocess this walk beside Export GPX and confirm only that route is refreshed before shared totals reconcile; verify the action is disabled during an active walk, that a primary Overpass 504 transparently uses the independent fallback, and that full-history Reprocess recordings remains available in Options.
