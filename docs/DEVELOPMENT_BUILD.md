...
34. Exercise the v0.33.1 France-catalogue protocol in `docs/TESTING.md`: switch between rank-1, rank-100, rank-101, rank-500, overseas, Thonon-les-Bains, and unsupported communes; verify the 20/10 thresholds, then run the active-city historical scan twice and verify district expedition medal availability. Confirm that only the active city's markers are mounted and that the second unchanged scan completes without walking the saved-history catalogue.
