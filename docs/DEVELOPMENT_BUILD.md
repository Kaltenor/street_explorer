...
Reload the SDK 54 development client from Metro for the camera fix. Standalone installations require a rebuilt app; no remote build was requested for this change. Version/build metadata is 0.35.7 / 237. Native map readiness now includes a measured layout. Follow the current launch-location protocol in Testing on Apple Maps and Google Maps.

## Location-label readability — 0.35.6

The JavaScript label adds a themed backdrop with native box shadows under the existing opacity animation. Reload Metro in the SDK 54 development client; standalone builds need rebuilding to include this change. Version/build metadata is 0.35.6 / 236. No remote build was requested for this visual edit.

## Polygon overlay build — 0.35.5 / 235

`npm install` applies `scripts/patch-react-native-maps-polygon-order.js` after the existing Apple POI patch. Its MapKit change keeps updated explored polygons in React child order, leaving later Forbidden Zones above them. This native change needs a fresh iOS build 235 or newer; reloading JavaScript in an older client does not include it. Android and iOS Google Maps receive their z-index order from JavaScript. No build 235 has been run as part of this audit.

Run `npm run measure:polygons` for the synthetic partition/contour baseline. It does not report native rendering time; compare frame pacing and purple-layer ordering with the build 235 device protocol in Testing.
...
Android preview build [233](https://expo.dev/accounts/fufuste/projects/street-explorer/builds/af17555a-b2fe-4ce2-929b-977680902680) finished; [download APK](https://expo.dev/artifacts/eas/DlaJW0vMBfVJHkindRT3Idz-MtuOHYWLW6OpFaiTYk0.apk). iOS development build [233](https://expo.dev/accounts/fufuste/projects/street-explorer/builds/ed38c74a-adcf-4386-8eae-ed889a43992b) finished; [download IPA](https://expo.dev/artifacts/eas/rRQNPhAlP5F2XVaeoPwHxS803Ouhu-4tyPE2Rpno_Q4.ipa). It includes devices already registered to the current ad hoc profile.
