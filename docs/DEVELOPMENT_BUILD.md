...
30. Force-close and relaunch the map without touching it. Confirm the full wordmark is about 20% larger than the prior initial presentation, then touch the map once and confirm it returns to the established compact size for the rest of the session. Verify the medal/flag rail, objective ledger, navigation, and walking field log are four separate square stripes spanning the complete screen width while retaining their previous heights. Tap the medal body and integrated right-side flag independently, open each selected destination, then start and stop a walk; confirm no content clips at the edges and every primary target remains at least 44 points high.
...
32. Open Options and cycle through Explorator, Daylight, and Custom. Confirm Daylight immediately changes the full Options page, all map HUD stripes, walking controls, native MapKit tiles, routes, territory, Details, History, Completion, Medals, diagnostics, recovery, and recording dialogs to the high-contrast light palette. Confirm Custom remains selectable but intentionally uses Explorator visuals. Leave each of Custom and Daylight selected across separate force-close/relaunch cycles and confirm the checked radio state and complete visual mode persist. Repeat the selector with VoiceOver and larger text, confirming the full row is tappable and each label, description, and checked state is announced without clipping.
...
36. Open Details on a profile with existing expedition seals and confirm each contributes 200 Explorer Points retroactively. Complete each of the day's three expeditions where practical and confirm each unique seal adds exactly 200 points and one `+200 PTS` reward stamp without a daily cap below the three available choices. Relaunch to verify no duplicate credit. In Daylight, trigger selection, enclosure, and expedition stamps and confirm their light gold/parchment lettering and dark outline remain readable against the navy center; switch back to Explorator and confirm its established stamp palette is unchanged.
