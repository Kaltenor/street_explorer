...
20. Confirm the map shows all nine Lyon arrondissement outlines, with 2-point burnt-orange neighboring strokes; a selected district uses a 6-point burnt-orange stroke and a selected city uses a 6-point red stroke, plus the compact Lyon medal progress card and one side flag. Pan across district boundaries and confirm the objective, correctly typed orange/red outline, and percentage do not change. Long-press another district, confirm haptic feedback and an immediate objective change, then use the District/City scope buttons to switch from the district's dark-orange boundary to the city's red boundary. Rapidly long-press different areas and confirm only the final selection and percentage survive. Tap the flag twice to show and hide the objective without clearing it. Verify layers remain configurable in Options and technical recording data remains available through History > Diagnostics or a recording's Technical details.
