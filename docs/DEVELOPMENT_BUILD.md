...
20. Confirm the map shows all nine Lyon arrondissement outlines, with muted neighboring strokes and a gold objective stroke, plus the compact Lyon medal progress card and one side flag. Pan across a district boundary and confirm the objective switches only after the 400ms settle interval, displays Calculating, and never accepts an older percentage after rapid back-and-forth movement. Tap the flag twice to show and hide the objective without clearing it. Verify layers remain configurable in Options and technical recording data remains available through History > Diagnostics or a recording's Technical details.
