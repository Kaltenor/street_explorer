...
## Android fresh installation — 0.35.1

Prerequisites: Android preview APK 231, internet, precise location permission, and a fresh profile (or an area whose boundaries are not cached). Do not erase existing walks for this test.

1. Launch near Lyon: the Explorator map should be dark and the first-location lookup should select a city/district after the service responds. Launch remains usable while loading.
2. Long-press Lyon or Villeurbanne: the selected area and available album should update. Select Daylight and Explorator: Google tiles should switch appearance.
3. Reopen: the saved objective and appearance should persist. With network disabled, cached areas should still select.
4. Hold an uncached area while offline: after bounded retries, an explanatory alert should appear. Restore internet and hold again to retry.
5. Start a walk and verify GPS, route drawing, Stop, and saved history. A stale GPS fix should not prevent selecting an area by holding the map.

Automated checks cannot establish device GPS, Google rendering, or live boundary-service availability.

## iOS Map Provider Switch — 0.35.0

Prerequisites: registered physical iPhone with the new build 230 development client (or newer), Metro running via `npx expo start --dev-client` on a reachable computer, active Google Maps iOS key configuration, internet for map tiles, and location permissions for recording. Use existing saved walks/medals/exploration if available. Native Google support requires a rebuilt client; a Metro reload is insufficient.

1. On a fresh preference, open Options. Expected: Apple Maps is selected. On an existing installation, existing walks/medals remain intact and Apple stays the default until explicitly changed.
2. Pan and zoom somewhere away from the player. Open Options → Map provider → Google Maps and return. Expected: Google map tiles load at the same center/zoom; district/city borders, explored territory, Forbidden Zones, route markers, player, and medals remain correctly placed. Switch back to Apple and repeat several times without duplicate overlays or camera jumps.
3. Select Google, close and reopen the app through the development client, and reconnect Metro if necessary. Expected: Google remains selected. Repeat with Apple. Cold launch may center on the startup GPS fix as usual; only the provider preference is persisted by this feature.
4. For each provider, try Explorator and Daylight and start a walk outdoors. Expected: map appearance matches the selected mode, labels stay readable, and the speech bubble's arrow touches the player's hat. Open Options during recording; provider buttons must be disabled. Lock the phone and walk several minutes, reopen, then Stop/save. Expected: continuous route and correct saved walk; switching becomes available after finalization.
5. While idle, deny location and relaunch; both provider choices remain usable for browsing. Disable internet and switch; uncached tiles may be blank, but local exploration and saved data remain. Reconnect and expect tiles to load. Restore location permissions before the next recording test.
6. With an older compatible iOS development client, load the new bundle. Expected: Apple Maps remains usable; Google is disabled with an install-new-build explanation, even if the saved preference was Google. No missing-native-module crash occurs. Reinstall the new client afterward.

Automated provider regressions run with `npm run test:map-provider` and are included in `npm test`. Actual Google/Apple rendering, device permission flows, and locked-screen recording require these manual checks; they are not proven by bundle export or EAS build success.

## Android Google Maps — 0.34.8

Prerequisites: physical Android phone with Google Play services, internet and location enabled, and a freshly built preview APK with the restricted Maps SDK key configured as described in [Development Build](DEVELOPMENT_BUILD.md#android-google-maps-and-standalone-apk). Google Cloud activation and Android device checks are not implied by passing automated tests.

1. Install the APK and launch directly without Metro. Expected: the app opens and Google map tiles load; no blank map or API-key error occurs.
2. Grant precise foreground location, then background location when requested for recording. Expected: the player centers on the usable fix. Deny location once and reopen; the app must remain usable without crashing, then recover after restoring permission.
3. Pan/zoom and select a district. Expected: boundaries, explored surfaces, player, and speech render correctly on Google Maps. Verify both appearances.
4. Start a walk outdoors, move at least 200 m, lock the screen for several minutes, then reopen. Expected: recording remains active with a continuous route and updated distance. Stop and reopen again; the saved walk and explored area persist.
5. Disable networking during another walk. Expected: GPS recording continues; uncached map tiles may be unavailable. Restore networking and confirm tiles return.

Only physical-device testing can establish rendering and background-recording behavior on the friend's Android model.

## Responsive city/district holds — 0.34.7

Prerequisites: physical iPhone, existing SDK 54 dev client reloaded with 0.34.7, a restored history, cached boundaries for two cities and several districts. Keep location access for normal map positioning; internet is needed only for uncached areas. Record visible response times rather than assuming a latency target has been met.

1. Hold two different displayed districts, then repeat offline. Expected: recognition gives selection feedback, the selected name/boundary changes without waiting for network or completion statistics, and progress can fill in afterward. Pan normally to check gesture coexistence.
2. Hold a cached city with no matching district, including offline. Expected: the city selects without an Overpass wait. A missing district may require an explicit boundary refresh from Completion before it becomes selectable.
3. Select an uncached area with a slow connection, then hold a known district or select an objective from Completion. Expected: the latest selection wins, loading clears, and the late old request never changes the objective. Enter Forbidden Zone mode during the request and verify the obsolete selection stays canceled.
4. Switch between two cached cities. Expected: only the selected city's boundaries remain, explored territory persists, and the existing City/District choice still works. Reopen and expect the selected objective to persist.
5. Repeat a countryside hold and an ocean hold. Expected: confirmed countryside clears the urban objective; ocean does not select anything. First-time lookup may still require network.
...
Prerequisites: an existing SDK 54 iPhone development client receiving the 0.34.4 Metro bundle; a verified external V5 backup from before removal; known walk, medal, completion and old seal counts; sufficient free space. Use a disposable profile for interruption tests. Allow location for recording; install referenced country packs before offline restore.

1. Stop any recording and fully reload the app. Expected: six destinations (Map, Details, History, Completion, Medals, Options), no expedition journal/HUD/action, and a usable map. Tap every tab in both languages and at larger text sizes; verify labels, spacing, touch targets, and Back/swipe behavior.
2. Compare History, explored surface, medals, Forbidden Zones, and zone completion with the baseline. Expected: all remain; score loses exactly 200 points per old seal when the underlying exploration is unchanged. Close and reopen; expect the same totals without repeated score changes.
3. Restore the old verified V5 archive on the disposable profile, then reopen offline. Expected: walks, frozen routes, medals, achievements, and Forbidden Zones return; expeditions and bonuses do not. A missing foreign pack must fail preflight before replacing data.
4. Export and verify a new V5 archive, restore it on the disposable profile, and reopen. Expected: the remaining data and score round-trip unchanged. Cancel a restore preview; expect no data change.
5. Record a short walk, background/foreground the app, close a qualifying enclosure, and Stop. Expected: one saved walk, normal explored-tile/enclosure points and reward, normal medal awards and completion, with no mission errors. Reopen to confirm persistence.
6. On a prepared disposable legacy database, interrupt upgrade or restore and relaunch. Expected: transactional rollback or a complete commit, no mixed dataset, and safe retry. Do not use the only copy of a real history for this test.
...
3. Select Mamoudzou, Nouméa, Papeete, and Saint-Martin. Expected: each overseas department or collectivity resolves its own album and exact OSM boundary, without falling back to a metropolitan city; Mamoudzou and Nouméa contain at least 20 medals because they rank in the top 100.
4. Select Thonon-les-Bains. Expected: its stable `thonon-les-bains-v1` album remains at 10 balanced medals and is now identified at official rank 211 rather than as an unranked exception.
5. Select a supported district whose parent is one tested city, then a commune outside the top 500. Expected: the district keeps its parent-city album; the unsupported commune shows no unrelated markers or progress.
6. Explore one uncollected anchor directly or through a valid enclosed surface. Expected: it unlocks once, persists after force-close/reopen, and appears under the correct city in All Cities. Run the historical scan twice; the second unchanged run should process no walks and create no duplicate.
7. Disable networking, force-close, and switch among three cached supported cities. Expected: all album definitions, collection state, filters, and markers remain available offline, while an uncached boundary may still require network.
8. Pan and switch rapidly among dense top-100 and rank-500 albums. Expected: only the active city's markers mount, stale progress never flashes, and launch/selection remains usable with the 500-album manifest.

Automated coverage validates 500 ranked communes, overseas department and collectivity presence, monotonic populations and ranks, 20/10 thresholds, 6,082 globally unique finite anchors, manifest lookup, source identities, storage-report consistency, type safety, and medal geometry. Physical-device validation remains required for native boundary parenting, MapKit marker replacement, real GPS awards, upgrade persistence, offline presentation, and perceived launch/switch performance.

## Thonon-les-Bains 10-Medal Catalogue V0.32.0 Manual Test

Prerequisites: install development build 218 or newer on a physical device, select the official Thonon-les-Bains commune boundary, and cache that boundary. For unlock coverage, use a real walk or test data whose explored surface reaches one listed landmark; network access is not required after the bundled app and boundary are installed.

1. Select Thonon-les-Bains and open Medals. Expected: the single `thonon-les-bains-v1` album loads with 10 medals and does not merge with a neighbouring commune.
2. Open each category filter. Expected: History, Architecture, Art, Culture, and Nature each contain exactly two medals, with readable French and English names and descriptions.
3. Inspect all 10 markers against the official commune boundary. Expected: Château de Ripaille, Château de Rives, Église Saint-Hippolyte, Basilique Saint-François-de-Sales, Musée du Chablais, Écomusée de la Pêche et du Lac, Fontaine de l’Hôtel de Ville, Monument des Allobroges, Parc de la Châtaigneraie, and Forêt de Ripaille all focus within Thonon-les-Bains.
4. Select a playable district or neighbourhood whose parent city relation is Thonon-les-Bains. Expected: the same Thonon album remains active; selecting a neighbouring commune switches away instead of leaking Thonon rewards.
5. Explore one medal anchor directly or enclose it through the normal validated-surface rules. Expected: it unlocks once, presents normally, remains collected after force-closing and reopening, and appears under Thonon-les-Bains in All Cities.
6. Disable network access, relaunch, and return to Thonon-les-Bains. Expected: all 10 definitions, collected state, filters, and locally cached boundary behavior remain available offline.
...
Prerequisites: install development build 216 or newer on a physical device, select Marseille, and cache its official city and arrondissement boundaries. For upgrade coverage, first collect or seed at least one Marseille medal with v0.30.4; for unlock coverage, use a real walk or a test profile whose directly explored or safely enclosed cells include a listed landmark. Network access is not required once the bundled app and boundaries are available.

1. Upgrade the existing profile and open Marseille's Medals page. Expected: the same `marseille-v1` collection is upgraded in place to definition version 2, previously collected medals remain collected, no duplicate album appears, and All shows 38 medals.
2. Open each arrondissement filter from 1 through 16. Expected: every arrondissement is present and contains at least one medal; central districts may contain more, while the 13th and 14th each contain one reviewed landmark.
3. Open the category filters. Expected: the complete roster contains 9 History, 8 Architecture, 6 Art, 8 Culture, and 7 Nature medals, with localized names and descriptions and no street, transit-stop, or generic district entries.
4. Select representative outer-district medals such as Parc de la Mirabelle (12th), Musée du Terroir Marseillais (13th), Bastide de Montgolfier-la-Tour-du-Pin (14th), and Villa La Palestine (16th). Expected: each marker focuses inside its stated arrondissement and locked cards reveal no reward-only description or article action.
5. Walk through, or load test exploration covering, one new landmark anchor. Expected: normal direct-cell or validated-enclosure rules unlock it once; reopening the app preserves the award and the All Cities view reports it under the single Marseille album.
6. Disable network access, relaunch, and switch repeatedly between Marseille arrondissements and another bundled French city. Expected: all 38 Marseille medals remain available offline, district filters and markers follow the current city, and the selection label dismisses without the previous long calculation stall.

Automated coverage validates the frozen definition version, exact count, preservation of all eight original IDs, complete 1–16 arrondissement coverage, five-category balance, stable identities, globally unique IDs, the 6,082-medal France total, and generator/source-manifest consistency. Physical-device validation remains required for MapKit marker placement, upgrade behavior with real collected data, and an actual location-based unlock.

## Marseille Objective/Medal Switch Repair V0.30.4 Manual Test
...
1. Open Completion, choose Marseille 7e Arrondissement, and set it as the objective. Expected: Completion closes promptly, the district-selection label dismisses after its normal brief animation, and the map remains responsive while completion finishes calculating.
...
3. Observe the medal rail and open Medals. Expected: the bundled Marseille album loads once, shows Marseille's 38 medals and saved progress, and never reports `Album unavailable` during overlapping refreshes.
...
Automated checks require indexed bounding-box reads for objective explored cells and district street candidates, use those reads in both Completion and the map HUD, and require one shared in-flight SQLite seed per medal album. TypeScript, zone, medal, full-suite, and iOS export results are reported in the release handoff. Physical-device verification remains required for perceived responsiveness and native MapKit marker replacement on the large real profile.
...
## Complete Medal Districts and All Cities V0.23.6 Manual Test

Prerequisites: use a physical-device development build upgraded to v0.23.6, with at least two unlocked medals in different supported cities and an active album that contains numbered districts (Paris or Lyon). Keep location and network access available for uncached boundaries, streets, or country packs; cache the chosen albums before the offline step. Do not begin with an active recording.
...
Automated checks cover medal grouping and collection persistence. Verify scrolling, map focus, Wikipedia, and reopening on an iPhone.
...
Automated checks cover the 44-item album version, exactly four new anchors per requested arrondissement, all five categories, localized copy integrity, globally unique identities, finite coordinates, the current 6,082-medal bundled-France total, type safety, and medal geometry. Physical-device verification remains required for visible MapKit placement, real GPS collection, upgrade preservation, offline relaunch, and performance with 44 mounted markers.
...
2. Enter the map. Expected: the district containing the launch fix is selected without a location label, its containing city is the only administrative context, the objective HUD names the district, and the district's parent-city medal album is active.
...
7. Force-close and reopen, then switch among three supported cities. Expected: every collected medal, presentation state, scan cursor, and city total persists independently; pending presentations still resume without loading all 100 albums into the visible map.
...
7. Repeat direct switching and highlighted-icon return in French, on the narrowest supported phone, and above the bottom safe area. Expected: all six icons and expanded labels fit without clipping; regular-width controls are 48 points high and below 360 points the visible widths compact responsively while five-point hit slop preserves effective 44-point touch targets. The footer coordinates remain identical and the last rows or buttons on every page can scroll fully above it.
...
1. From the map, open Details, History, Completion, Medals, and Options one at a time. Expected: each tap immediately plays one short, natural book-page turn at a discreet level approximately half the previous player volume; the former navigation cue is not heard.
...
4. From History, focus a saved walk; from Completion, focus or select an objective; and from Medals, focus a landmark. Expected: each action that returns to the map uses the same page-turn cue. Transfers between Atlas destinations, such as Details to History, do not layer duplicate sounds.
...
## Atomic Boundary Switching and Location Label iPhone Manual Test

Prerequisites: run the 0.34.5 bundle on an iPhone, allow foreground location, enable Explored Cells, cache boundaries for Lyon and Villeurbanne, and keep some explored territory visible in Lyon. Network is required only for uncached OSM boundaries or MapKit tiles.
...
5. Switch districts while the full top objective panel and bottom walking controls are visible, including once immediately after a cold launch. Expected: the discreet place label appears between the measured controls, rises gently with expanding rules, and fades without blocking the map.
6. Inspect the longest available title and district/city name over both light and dark map regions. Expected: the name fits within two lines, with readable day/night ink and a subtle halo, without a panel or seal.
7. Enable iOS Reduce Motion and select another district. Expected: the location label uses opacity-only entry and exit, without translation or expanding rules.
...
Automated coverage verifies one polygon per district, exactly one conditional selection fill, keyed administrative context replacement, atomic city/district state, and measured free-map-space placement. Physical-device verification remains required for MapKit native-overlay teardown, exact safe-space placement across iPhone sizes, haptics, sound, and Reduce Motion.
...
Prerequisites: run Street Explorer 0.16.27 in a compatible iOS development client (build 142 or newer for a matching release binary), allow foreground location, and prepare a qualifying enclosure. Start a music track and then a spoken-word podcast in another app so both content types can be checked. Headphones and the phone speaker should each be tested when practical; no network is required for the bundled sounds.
...
2. Switch city or district. Expected: its quiet piano cue accompanies the location label and plays over the external audio; the other app continues uninterrupted.
...
4. Trigger another reward after the first jingle has ended. Expected: both retained players restart from their beginning; no cue is missing, delayed by player creation, or resumed from its previous end position.
5. Repeat steps 2–5 with Reduce Motion enabled, Bluetooth/headphones when available, and both music and podcast playback. Expected: the visual motion changes as documented, but cue order, prompt start, and external-audio mixing remain consistent.
...
Prerequisites: run Street Explorer 0.16.26 in a compatible iOS development client (build 141 or newer for a matching release binary), allow foreground location, and use a walkable location where a qualifying enclosure can be closed. Network is needed only for uncached MapKit, boundary, street, or medal data; the reward assets themselves are offline.

1. Select a district or city objective. Expected: the discreet location label uses its soft piano cue without a victory jingle.
2. Start a walk and close a new qualifying enclosure. Expected: the enclosed surface fills, one large map-centered `AREA ENCLOSED` stamp retains its established large scale/strike and reports the number of newly revealed cells, the dedicated victory jingle and medium haptic play once, and recording/map interaction continues without pausing.
...
4. Enable iOS Reduce Motion and repeat a qualifying closure. Expected: the reward remains visible for the reduced duration, uses the final stamp state instead of the strike sequence, continues to avoid blocking input, and still plays the reward audio/haptic.
5. Switch the app to French and repeat the enclosure reward. Expected: localized `ZONE ENCLOSE` wording fits inside the existing stamp, including the singular/plural revealed-cell detail.
...
Prerequisites: run the Street Explorer 0.34.4 bundle in a compatible SDK 54 development client, allow Files access, stop any active recording, keep one known-good verified V5 archive, and ensure the device has enough free space for a backup plus a GPX ZIP. For archive-block coverage, use a database with at least 25 finalized walks. Include named walks with accents and filesystem-reserved punctuation if practical. Location permission is not required for export or restore. Restore requires network access if the backup references a foreign-country medal pack not installed on this device.
...
8. Tap Restore and choose the verified V5 archive. Expected: Checking backup appears while the complete archive is validated; only afterward does a confirmation show export date, source app version, size, walk count, GPS-point count, medal count, zone-achievement count. Local data has not changed.
...
11. Select the valid V5 archive again and confirm its preview. Expected: Restore in progress appears immediately, repeated data-tool taps are disabled, the same archive is revalidated, and all logical walks return with their original IDs/names/times/counts, frozen route geometry and inferred evidence, medals, zone achievements; no monthly archive block appears as a fake recording.
...
## Active Recording Auto-Recovery Test

Prerequisites: install development build 214 or newer on a physical iPhone, grant precise foreground and Always background location, and prepare outdoor routes with reliable GPS. Internet access is optional because recovery uses local SQLite data and native task state. Record the session ID from diagnostics when comparing pre/post-relaunch state.

1. Start a walk, collect several valid points, note distance and elapsed time, then close or development-reload the app without pressing Stop. Reopen it. Expected: the launch gate never exposes Start first; the same session opens directly in the normal active HUD, the old path/distance remain, elapsed time uses the original start timestamp, and a short “Walk resumed after interruption” notice appears. Continue, Stop, and expect exactly one History row containing both portions.
2. Repeat with a substantial real route, closing near its midpoint and continuing after automatic recovery. Expected: every recoverable pre/post-interruption point remains under the original session ID and final start/stop times, trusted distance, cells, statistics, and available steps belong to that one row.
3. Record section A, fully terminate the app, move a meaningful distance while it cannot receive locations, reopen, and record section B. Expected: A and B remain one session, the unavailable interval splits the live route, and the missing corridor contributes no straight route, trusted distance, explored cells, loop boundary, or completion credit.
4. Start a walk, background and lock the iPhone, walk, then reopen without termination. Expected: journaled background points synchronize into the same session in timestamp/index order; foreground and background delivery of the same timestamp does not duplicate points, distance, or cells.
5. Repeat close/reopen three times before Stop. Expected: every relaunch resumes the same ID, only one foreground watcher/background owner records, duration continues from the first Start, and final History still has one row.
6. Stop normally, relaunch, and expect no automatic recovery or Recovery V2 modal. Start must create a new session only after initialization completes.
7. Use the Recovery V2 fallback to Discard an unfinished test recording, relaunch, and expect it not to return. Generic app backgrounding, termination, or React unmount must never invoke Discard.
8. Launch with no active recording. Expected: startup behaves normally, no recovery notice/modal appears, and Start becomes available only after the recording-state check completes.
9. Deny foreground location or simulate failure while stopping/restarting native tracking during recovery. Expected: the unfinished SQLite session remains authoritative and Recovery V2 appears with Resume, Finish & Save, and Discard; retrying Resume never creates a second session.
10. In the fallback, verify Active/Interrupted/Uncertain status, bounded full-route preview, editable date/time name on Finish, compare-clearing only after durable finalization, and safe preservation after failed Resume/Finish/Discard.
11. Repeat the automatic and fallback cases offline and with a route longer than 300 points. Expected: recovery does not need network access, all persisted points affect reconstruction, and only diagnostic/raw React state is tail-bounded.

Automated checks: `npm run typecheck` and `npm run test:geometry` cover automatic restoration wiring, launch gating, persistent cold-callback ownership, immediate foreground persistence admission, canonical timestamp deduplication, same-session Stop ordering, and rejected outage gaps contributing neither exploration nor trusted distance. Physical-device verification remains required for iOS process termination, native task survival/restart, Always permission behavior, lock-screen delivery, pedometer reconstruction, and real foreground/background callback timing.
...
2. Start a walk and inspect the Field Log Forbidden Zone map-with-slash control. Expected: the control is visibly disabled and cannot enter selection mode. Stop and save the walk; expected: the control becomes available.
3. Tap the Forbidden Zone control while idle. Expected: its purple selected state is unmistakable and the Field Log says to long-press a surrounded inaccessible area. Tap it again; expected: the mode exits without changing the objective or data. Reactivate it for the remaining steps.
4. While the mode is active, long-press a different city or district. Expected: only Forbidden Zone handling runs; neither the city nor district objective changes and the District/City picker does not appear. Deactivate the tool and repeat the same hold; expected: the original administrative selection behavior returns.
...
5. Long-press the open test region. Expected: no zone is created, the mode stays active, and the app says the area has not been completely surrounded.
6. Surround and reprocess the sub-150,000 m² region. Expected: normal loop fill captures it as orange explored territory. Activate Forbidden Zone mode and hold inside the underlying enclosure. Expected: no purple zone is created and the app explains that normal capture applies.
7. Reprocess the 150,000 m²–2 km² enclosure. Expected: loop diagnostics reject it as `loop_area_too_large` and its interior remains unexplored. Activate Forbidden Zone mode and hold inside it. Expected: the app shows a checking state, creates one immutable region matching the enclosure rather than a radius, exits selection mode, immediately renders a translucent-purple fill/darker contour, and confirms that the area is excluded—not explored—with m² or km² formatting.
...
9. Force-close and reopen. Expected: the same purple geometry returns from SQLite without reselecting or expanding. Explore adjacent cells, reopen again, and confirm the stored purple snapshot does not grow.
10. Select an overlapping district and record its raw cached cell total, explored numerator, forbidden overlap, eligible total, and percentage. Expected: `eligible = raw total - forbidden overlap`; the explored numerator is unchanged and percentage uses `explored / eligible`.
11. Select a Forbidden Zone that crosses two cached districts or a city boundary. Expected: each scope excludes only cell centers inside its own exact polygon; the zone has no permanent district/city ownership and remains unchanged after a boundary refresh.
12. Activate the tool and hold inside the purple zone. Expected: Remove Forbidden Zone? explains that the area will count again. Choose Cancel; expected: the mode and purple zone remain. Repeat, choose Remove, and expect the purple layer to disappear, the eligible denominator/remaining count to restore, the current percentage to recalculate, and selection mode to exit. Immediately reactivate the tool and recreate the same enclosure; expected: it succeeds without an overlap warning. Also upgrade a database affected by the old orphan-cell bug; expected: migration 35 repairs it at launch and the enclosure can be created again.
13. Verify removal did not delete or alter GPS points, recordings, route snapshots, explored cells, loop fills, Explorer Score, or a previously earned permanent district/city achievement. Expected: the permanent achievement remains earned even if current percentage is now below 100%, while the live percentage and remaining count show the restored denominator.
14. Recreate a valid Forbidden Zone, delete an unrelated recording, and run both single-walk and full Reprocess recordings. Expected: the zone and its completion effect survive every operation while explored/loop-derived data rebuilds normally.
15. Refresh OSM/admin boundaries. Expected: the local Forbidden Zone remains; completion recomputes overlap against the refreshed polygons instead of using a stored administrative attachment.
16. Attempt a completely enclosed selection larger than 2 km². Expected: no zone is stored, the mode stays active, and the app reports that the selected area is too large.
17. Export and externally verify a new Backup V5 with at least one Forbidden Zone, clear local data, then restore it. Expected: exact cells, area, purple geometry, and completion denominator effects return once, alongside existing recordings and progress.
18. Export and restore a v0.30 Backup V5. Expected: Forbidden Zone comments return with exact geometry and cells. Restore the v0.29 archive whose zone has no `comment`; expected: validation succeeds and the zone returns with an empty optional comment. Restore the pre-v0.29.0 Backup V5 that has no `forbiddenZones` field; expected: validation succeeds, Forbidden Zone state is empty, and all older supported data restores normally.
...
## Resilience audit regression checks

Run `npm run test:resilience` on Node 24, or `npm test` for the complete aggregate. The focused suite executes real service logic with synthetic data and in-memory SQLite adapters: stalled-body deadlines and fallback, cancellation retry, historical street-date reconstruction, migration rollback/retry, backup yielding/corruption, and randomized incremental/full geometry equivalence. It does not emulate native device termination or MapKit. Follow [the audit device protocol](AUDIT_FOLLOWUP.md) for remaining physical checks.
