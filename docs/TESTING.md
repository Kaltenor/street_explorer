...
Prerequisites: use a physical-device development build upgraded to v0.23.11. Before upgrading, identify at least one still-locked landmark whose center lies inside an orange validated surface but not on an individually walked 15 m cell, ideally in each of two supported cities; install the relevant downloadable country pack beforehand if one target is outside France. Keep a second country pack absent for the no-download check. Finish any active recording, then disable networking before the first upgraded launch.
