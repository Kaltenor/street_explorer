...
- CC0 static pixel-art player marker with one explicitly sized image child anchored directly by MapKit, one-time Start/Resume recentering, camera-independent panning, durable trustworthy-position restore across app/session relaunches, accessible stale-GPS state, and continuous Stop/Start/recovery visibility. Screen-space projection, coordinate animation, direction, and frame animation remain disabled to preserve device-stable rendering. Stop releases the summary and next Start at the durable save boundary while repairable derived work continues asynchronously, and continuous routes bypass unnecessary street-corridor inference.
