...
`explorerScore.ts` derives score from the authoritative cell sets instead of persisting a mutable counter. It removes known `loop_fill` cells from the walked set, re-derives every currently fillable contour under the gameplay area cap, unions persisted and derived enclosure cells, then awards one point for each unique discovered cell plus one enclosure bonus for each enclosed cell. This makes historical coverage score immediately, lets active cells update the total live, and stays duplicate-safe across reprocessing, backup restore, relaunch, and later reconciliation. `areaComparisons.ts` bundles 55 sorted international reference surfaces and their source URLs so `ExplorerScorePanel` can show a current equivalence and next milestone without network access. `MapScreen` passes the same derived total to Details, the idle Field Log, and enclosure-stamp point labels. The idle `WalkControls` row pairs that score with today's pedometer total; an active recording replaces the idle pair with distance, duration, and current-walk steps. Start and Stop share one centered 60%-width, 44-point-high control, followed immediately by the GPS status strip in both states.
...
`district_expeditions` stores the deterministic daily choice identity, district/date/slot, goal kind and target, durable progress, and acceptance/abandonment/completion timestamps. A partial unique index permits only one accepted unfinished expedition globally. `district_expedition_seals` permanently records completed missions. `district_expedition_loop_evidence` links a detected loop to its recording; progress queries join only finalized sessions, and discarding an underfilled recording removes provisional evidence through its foreign key. Cell, street, and medal goals are recalculated from durable post-acceptance timestamps, so reopening or repair cannot double-count. Acceptance is blocked during recording to make that timestamp boundary unambiguous.
...
The Medals screen still offers an explicitly confirmed cumulative scan of saved walks for older coverage and records completion per frozen album version. The one-time v0.5 repair is narrower: it only restores awards that an individual saved recording should have earned under the live/Stop rules. Backup V5 includes acquisition events, collected state, presentation state, and retro-scan settings. V1-V4 restore support and the temporary V4 conversion bridge have been removed.
