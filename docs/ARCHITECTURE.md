...
Large exact zones can intentionally show a pending denominator to avoid expensive country-scale scans on the phone. Completion augments persisted cells with the same walking area-capped enclosed contour cells used by the solid red renderer, so a qualifying visible surface and its percentage use the same numerator. While recording, the objective HUD merges the unique active GPS cells into that numerator and recalculates when the live cell set changes, allowing a newly closed contour to update immediately. This live calculation is a read-only preview: it cannot persist a 100% achievement until finalization makes the cells durable, and the finalized refresh invalidates older preview requests before publishing. Local scans are chunked, yield to the React Native event loop, begin after the opening transition, and abort when Completion closes.
