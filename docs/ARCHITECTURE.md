...
The offline France catalogue contains one frozen JSON album for each of the 100 largest metropolitan communes in the INSEE 2023 municipal-population ranking, totalling 851 medals. Paris v2 carries 60 reviewed anchors with explicit coverage of all 20 arrondissements; Lyon v1 and Villeurbanne v1 retain their 20- and 14-item hand-curated rosters. Generated albums freeze Ministry of Culture Mérimée/Muséofile anchors with reviewed Wikidata fallback identities. A generated manifest keeps rank, INSEE code, OSM city relation, definition version, and medal count separate from lazy album loaders. The registry resolves a city objective directly or a district objective through its parent city and materializes only that definition. Unsupported or absent city objectives return no album rather than leaking another city's rewards.

SQLite migration 29 adds album/city spatial metadata, coordinate indexes, and a `walk_session_bounds` table maintained as GPS points arrive. Its initial bounds backfill joins GPS points to existing parent sessions, so detached rows left by older databases are ignored instead of violating the new bounds-table foreign key. `ensureBundledMedalAlbumSeeded` compares definition version, city relation, and bounds before writing; unchanged app startup performs no catalogue-definition writes. Collected evidence and presentation state remain separate, and pending-presentation startup first queries distinct referenced album ids instead of loading every album. The collection view preserves permanent Unlocked and Locked sections in frozen catalogue order.
