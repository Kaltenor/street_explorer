...
## Current shipped medal contract (v0.28.11)
...
- City and parent-district objectives resolve one active album through the generated manifest; unsupported cities receive no fallback album. The launch discovered-area pass materializes every bundled definition plus already-installed country packs but seeds only albums containing an actual match.
- A medal unlocks whenever its anchor occupies a directly discovered 15m tile or any enclosed cell in the same validated, 150,000m2-capped surface rendered by the map. Active-city direct and contained-surface matches can award live. Normal single-recording closure remains an additional path with its 80m minimum, exact-contour-first plus one-cell seam tolerance, accepted finalized inferred geometry, and strict-interior anchor.
- Every launch idempotently reconstructs the validated surface from persisted discovery, checks all locally available city albums against it, and queues new awards before launch readiness. This pass is keyed to the persisted exploration revision, retries after failure, and serializes matching-album writes. The separate recording-history scan remains explicit, per-album, spatial, incremental, and definition-versioned; an unchanged scan loads zero walks.
