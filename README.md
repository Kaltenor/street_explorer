...
The app supports an updated shared portrait splash/launch presentation, self-healing foreground/background recording, accuracy-aware startup and walking-scale recording-start centering, a persistent four-direction pixel-art player overlay with native location fallback, hole-free full-walk live rendering, bounded durable late-GPS recovery, walking-focused GPS filtering, full-screen Details/History/Completion/Medals views, explored cells, loop-fill analysis, landmark medal collection, safe backup/restore, device step counts, city-wide district outlines with a map-driven objective HUD, and persistent OpenStreetMap street completion derived from frozen saved routes.
