...
Midnight Cartographer gives the iOS playfield the same identity as the navy/gold interface: Apple MapKit uses a dark muted presentation, generic POIs and the native blue cursor are hidden, and only game-owned landmarks, route markers, and the player remain. Translucent light orange means explored territory, gold marks today's discoveries and the active walk, parchment highlights a selected saved route, inferred links remain bright teal, and other saved walks use restrained atlas colors. District boundaries use thicker burnt-orange outlines and selected city boundaries use red, while start, finish, and medal locations use custom hand-inked parchment markers instead of native pins.

Atlas Cabinet extends the medal menu's identity to Details, History, Completion, and Options with gold titles, cartographic emblems, ornamental dividers, subtle navy paper texture, quiet page/ink sounds, and Reduce Motion-aware transitions. Newly selected or completed objectives receive a restrained map stamp, new explored cells flash as fresh orange ink, and a focused saved route draws onto the map before its finish marker appears.
